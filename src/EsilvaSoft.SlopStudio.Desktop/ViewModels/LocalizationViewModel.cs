...
public sealed partial class LocalizationViewModel : ObservableObject
{
    ...
    [
        ...
        ("aiAssistant", "AI assistant", "Assistente IA", "Asistente de IA", "AI 助手"),
        ("perTab", "per tab", "por aba", "por pestaña", "按标签页"),
        ("aiContextInfo", "AI receives the instruction, editor and context from this tab. Nothing is executed automatically.", "A IA recebe a instrução, o editor e o contexto desta aba. Nada é executado automaticamente.", "La IA recibe la instrucción, el editor y el contexto de esta pestaña. Nada se ejecuta automáticamente.", "AI 会接收此标签页的指令、编辑器内容和上下文。不会自动执行任何内容。"),
        ("aiIntro", "Analyze, create or refactor code with a reviewable proposal.", "Analise, crie ou refatore o código com uma proposta revisável.", "Analice, cree o refactorice código con una propuesta revisable.", "使用可审阅的提案分析、创建或重构代码。"),
        ("conversation", "Conversation", "Conversa", "Conversación", "对话"),
        ("review", "review", "revisão", "revisión", "审阅"),
        ("proposedCode", "Code proposed by AI", "Código proposto pela IA", "Código propuesto por la IA", "AI 提议的代码"),
        ("applyProposal", "Apply proposal", "Aplicar proposta", "Aplicar propuesta", "应用提案"),
        ("dismiss", "Dismiss", "Descartar", "Descartar", "忽略"),
        ("chatPlaceholder", "Ask for an analysis or change…", "Peça uma análise ou alteração…", "Pida un análisis o cambio…", "请求分析或修改…"),
        ("aiInstruction", "Instruction for AI chat", "Instrução para o chat com IA", "Instrucción para el chat con IA", "AI 聊天指令"),
        ("aiNoAutoApply", "The proposal is never applied automatically.", "A proposta nunca é aplicada automaticamente.", "La propuesta nunca se aplica automáticamente.", "提案永远不会自动应用。"),
        ...
        , ("localProcessingNote", "Local AI runs on this device. Chat context is opt-in and is sent only to the selected local model.", "A IA local roda neste dispositivo. O contexto do chat é opcional e só vai para o modelo local selecionado.", "La IA local se ejecuta en este dispositivo. El contexto del chat es opcional y solo se envía al modelo local seleccionado.", "本地 AI 在此设备运行。聊天上下文需选择启用，且只发送到所选本地模型。")
        ...
        , ("useLocalModelAssistant", "Use local model in AI assistant", "Usar o modelo local no Assistente IA", "Usar el modelo local en el Asistente de IA", "在 AI 助手中使用本地模型")
        , ("allowLocalAiContext", "Allow AI assistant to use tab context with the local model", "Permitir que o Assistente IA use o contexto da aba no modelo local", "Permitir que el Asistente de IA use el contexto de la pestaña en el modelo local", "允许 AI 助手将标签页上下文用于本地模型")
        , ("localAiContextScope", "When enabled, the prompt may include editor code, database and collection names, and inferred field names from Results. It never includes credentials or result values. Input JSON is separate and off by default.", "Quando ligado, o pedido pode incluir o código do editor, nomes do banco e da coleção e nomes de campos inferidos dos Resultados. Nunca inclui credenciais nem valores dos resultados. O JSON de entrada é separado e começa desligado.", "Al activarlo, la solicitud puede incluir código del editor, nombres de base y colección y campos inferidos de Resultados. Nunca incluye credenciales ni valores de resultados. El JSON de entrada es independiente y está desactivado por defecto.", "启用后，请求可能包含编辑器代码、数据库和集合名称，以及从结果推断的字段名。不会包含凭据或结果值。输入 JSON 单独控制，默认关闭。")
        , ("allowLocalAiInputJson", "Also include Input JSON in local AI chat", "Também incluir o JSON do Input no chat de IA local", "Incluir también el JSON de Input en el chat de IA local", "在本地 AI 聊天中也包含 Input JSON")
        , ("localAiInputJsonScope", "Explicit opt-in. Input JSON is sent only to the selected local model and is limited to 8,192 characters.", "Opt-in explícito. O JSON do Input só é enviado ao modelo local selecionado e tem limite de 8.192 caracteres.", "Consentimiento explícito. El JSON de Input solo se envía al modelo local seleccionado y tiene un límite de 8.192 caracteres.", "需明确启用。Input JSON 仅发送到所选本地模型，最多 8,192 个字符。")
        , ("allowLocalAiConnectionContext", "Allow tab context for the local AI assistant on this connection", "Permitir contexto da aba no Assistente IA local para esta conexão", "Permitir el contexto de la pestaña en el Asistente de IA local para esta conexión", "允许本地 AI 助手在此连接中使用标签页上下文")
        , ("localAiConnectionContextScope", "Turn this off to block editor, target and Results context for this connection. Other connections keep their own setting. This context stays on this device and is sent only to the selected local model.", "Desligue para bloquear o contexto do editor, destino e Resultados nesta conexão. As outras conexões mantêm sua própria opção. O contexto fica neste dispositivo e só vai para o modelo local selecionado.", "Desactívelo para bloquear el contexto del editor, destino y Resultados en esta conexión. Las demás conexiones conservan su opción. El contexto permanece en este dispositivo y solo se envía al modelo local seleccionado.", "关闭后，此连接不会提供编辑器、目标和结果上下文。其他连接保留各自设置。上下文仅在此设备上，并只发送到所选本地模型。")
        , ("aiContextDisabled", "Local AI context is disabled. Enable it in Autocomplete and local AI settings and for this connection.", "O contexto da IA local está desativado. Habilite-o em Autocomplete e IA local e nesta conexão.", "El contexto de IA local está desactivado. Actívelo en Autocompletado e IA local y para esta conexión.", "本地 AI 上下文已禁用。请在自动补全和本地 AI 设置及此连接中启用。")
        , ("previewContext", "Review context", "Revisar contexto", "Revisar contexto", "查看上下文")
        , ("confirmSend", "Send to local AI", "Enviar à IA local", "Enviar a la IA local", "发送给本地 AI")
        , ("aiContextPreviewReview", "Review the exact content below before sending it to local AI. No model has been called yet.", "Revise abaixo o conteúdo exato antes de enviá-lo à IA local. Nenhum modelo foi chamado ainda.", "Revise abajo el contenido exacto antes de enviarlo a la IA local. Aún no se ha llamado a ningún modelo.", "请在发送给本地 AI 前检查以下准确内容。尚未调用模型。")
        , ("aiPreviewInstruction", "Instruction", "Instrução", "Instrucción", "指令")
        , ("aiPreviewHeader", "Header", "Cabeçalho", "Encabezado", "标头")
        , ("aiPreviewEditor", "Editor", "Editor", "Editor", "编辑器")
        , ("aiPreviewDatabase", "Database", "Banco de dados", "Base de datos", "数据库")
        , ("aiPreviewCollection", "Collection", "Coleção", "Colección", "集合")
        , ("aiPreviewOperation", "Operation", "Operação", "Operación", "操作")
        , ("aiPreviewLanguage", "Language", "Linguagem", "Lenguaje", "语言")
        , ("aiPreviewDialect", "Dialect", "Dialeto", "Dialecto", "方言")
        , ("aiPreviewAdditionalContext", "Additional context", "Contexto adicional", "Contexto adicional", "附加上下文")
        , ("aiContextPreviewChanged", "The instruction or context changed. Review a new preview before sending.", "A instrução ou o contexto mudou. Revise uma nova prévia antes de enviar.", "La instrucción o el contexto cambió. Revise una nueva vista previa antes de enviar.", "指令或上下文已更改。请先查看新的预览再发送。")
        , ("aiInputContextTooLarge", "Input JSON exceeds the 8,192 character limit for local AI context.", "O JSON do Input excede o limite de 8.192 caracteres do contexto da IA local.", "El JSON de Input supera el límite de 8.192 caracteres del contexto de IA local.", "Input JSON 超出本地 AI 上下文的 8,192 字符限制。")
        , ("aiEditorContextTooLarge", "Editor content exceeds the 1 MiB limit for local AI context. Shorten it before requesting a proposal.", "O conteúdo do editor excede o limite de 1 MiB do contexto da IA local. Reduza-o antes de pedir uma proposta.", "El contenido del editor supera el límite de 1 MiB del contexto de IA local. Redúzcalo antes de solicitar una propuesta.", "编辑器内容超出本地 AI 上下文的 1 MiB 限制。请缩短后再请求提案。")
        , ("aiInvalidProposal", "The proposal was rejected because its code is invalid: {0}", "A proposta foi recusada porque o código é inválido: {0}", "Se rechazó la propuesta porque el código no es válido: {0}", "提案代码无效，已拒绝：{0}")
        ...
        , ("aiNoContext", "Choose a database or write content in the editor to provide context to AI.", "Escolha um banco ou escreva conteúdo no editor para fornecer contexto à IA.", "Elija una base de datos o escriba contenido en el editor para proporcionar contexto a la IA.", "选择数据库或在编辑器中输入内容，为 AI 提供上下文。")
        , ("chatIntro", "Ask a question about this tab's code.", "Faça uma pergunta sobre o código desta aba.", "Haga una pregunta sobre el código de esta pestaña.", "询问有关此标签页代码的问题。")
        , ("aiAnalyzing", "Analyzing the instruction and tab context…", "Analisando a instrução e o contexto da aba…", "Analizando la instrucción y el contexto de la pestaña…", "正在分析指令和标签页上下文…")
        , ("aiStaleResponse", "The editor or target changed while AI was responding. The old proposal was discarded.", "O editor ou o destino mudou enquanto a IA respondia. A proposta antiga foi descartada.", "El editor o el destino cambió mientras la IA respondía. Se descartó la propuesta anterior.", "AI 响应期间编辑器或目标发生了变化。旧提案已丢弃。")
        , ("aiEmptyResponse", "AI did not return a proposal for this instruction.", "A IA não retornou uma proposta para esta instrução.", "La IA no devolvió una propuesta para esta instrucción.", "AI 未为此指令返回提案。")
        , ("aiProposalReadyConfirm", "Proposal ready. Read the warning and confirm again before applying.", "Proposta pronta. Leia o alerta e confirme novamente antes de aplicar.", "Propuesta lista. Lea la alerta y confirme de nuevo antes de aplicarla.", "提案已准备好。应用前请阅读警告并再次确认。")
        , ("aiProposalReady", "Proposal ready for review. Nothing changed in the editor.", "Proposta pronta para revisão. Nada foi alterado no editor.", "Propuesta lista para revisión. No se cambió nada en el editor.", "提案已准备好供审阅。编辑器未发生更改。")
        , ("aiAnalysisCancelled", "Analysis canceled. The editor was not changed.", "Análise cancelada. O editor não foi alterado.", "Análisis cancelado. El editor no se modificó.", "分析已取消。编辑器未更改。")
        , ("aiQueryFailed", "Could not query AI: {0}", "Não foi possível consultar a IA: {0}", "No se pudo consultar la IA: {0}", "无法查询 AI：{0}")
        , ("aiCanceling", "Canceling analysis… the editor remains unchanged.", "Cancelando a análise… o editor permanece intacto.", "Cancelando el análisis… el editor permanece intacto.", "正在取消分析…编辑器保持不变。")
        , ("aiProposalStale", "The proposal is outdated because the editor changed. Generate a new proposal.", "A proposta ficou desatualizada porque o editor mudou. Gere uma nova proposta.", "La propuesta está desactualizada porque cambió el editor. Genere una nueva propuesta.", "提案已过期，因为编辑器发生了变化。请生成新提案。")
        , ("aiProposalApplied", "Proposal applied to the editor. Use Ctrl+Z to undo.", "Proposta aplicada ao editor. Use Ctrl+Z para desfazer.", "Propuesta aplicada al editor. Use Ctrl+Z para deshacer.", "提案已应用到编辑器。使用 Ctrl+Z 撤销。")
        , ("aiStatusApplied", "AI proposal applied", "Proposta de IA aplicada", "Propuesta de IA aplicada", "AI 提案已应用")
        , ("aiAdditionalConfirmation", "This proposal requires additional confirmation because it contains a write or destructive operation.", "Esta proposta exige uma confirmação adicional porque contém escrita ou operação destrutiva.", "Esta propuesta requiere confirmación adicional porque contiene una operación de escritura o destructiva.", "此提案包含写入或破坏性操作，需要额外确认。")
        , ("aiProposalDismissed", "Proposal discarded. The editor was not changed.", "Proposta descartada. O editor não foi alterado.", "Propuesta descartada. El editor no se modificó.", "提案已丢弃。编辑器未更改。")
        , ("noChanges", "No changes", "Nenhuma alteração", "Ningún cambio", "无更改")
        , ("proposalToReview", "Proposal to review", "Proposta para revisar", "Propuesta para revisar", "待审阅提案")
        , ("aiProposalSafety", "This proposal contains a write or destructive operation. Applying it requires additional confirmation and will not execute the code.", "Esta proposta contém escrita ou operação destrutiva. A aplicação exigirá uma confirmação adicional e não executará o código.", "Esta propuesta contiene una operación de escritura o destructiva. Aplicarla requiere confirmación adicional y no ejecutará el código.", "此提案包含写入或破坏性操作。应用它需要额外确认，且不会执行代码。")
        , ("aiProposalReviewHint", "Review the proposal. It is inserted into the editor only after your confirmation.", "Revise a proposta. Ela só será inserida no editor depois da sua confirmação.", "Revise la propuesta. Solo se insertará en el editor después de su confirmación.", "请审阅提案。只有在您确认后才会插入编辑器。")
        ...
        , ("remoteHintRecommendedCpuRam", "Recommended on CPU: better for AI Assistant, about 3.8 GB RAM", "Recomendado em CPU: melhor no Assistente IA, cerca de 3,8 GB de RAM", "Recomendado en CPU: mejor para el Asistente IA, unos 3,8 GB de RAM", "推荐用于 CPU：更适合 AI 助手，约需 3.8 GB 内存")
        ...
        , ("updateReady", "Update {0} ready. It will be installed when closing Slop Studio; click to restart now.", "Atualização {0} pronta. Será instalada ao fechar o Slop Studio; clique para reiniciar agora.", "Actualización {0} lista. Se instalará al cerrar Slop Studio; haga clic para reiniciar ahora.", "更新 {0} 已就绪。关闭 Slop Studio 时将安装；点击立即重新启动。")
        ...
        , ("updateReadyPrompt", "Version {0} will be installed when Slop Studio closes. Restart now? Running tabs and drafts follow normal close confirmations.", "A versão {0} será instalada quando o Slop Studio fechar. Reiniciar agora? Abas em execução e rascunhos seguem as confirmações de um fechamento normal.", "La versión {0} se instalará cuando se cierre Slop Studio. ¿Reiniciar ahora? Las pestañas en ejecución y los borradores siguen las confirmaciones normales de cierre.", "Slop Studio 关闭时将安装版本 {0}。现在重新启动？正在运行的标签页和草稿遵循正常关闭确认。")
        ...
        , ("proposalConfirmTitle", "Confirm proposal application", "Confirmar aplicação de proposta", "Confirmar aplicación de propuesta", "确认应用提案")
        , ("insertProposalPrompt", "Insert this proposal into the editor?", "Deseja inserir esta proposta no editor?", "¿Desea insertar esta propuesta en el editor?", "将此提案插入编辑器？")
        , ("proposalStaleEditorTarget", "The proposal is outdated because the editor or target changed. Generate a new proposal.", "A proposta ficou desatualizada porque o editor ou o destino mudou. Gere uma nova proposta.", "La propuesta está desactualizada porque cambió el editor o el destino. Genere una nueva propuesta.", "提案已过期，因为编辑器或目标发生变化。请生成新提案。")
        ...
        , ("aiContextConnection", "connection: {0}", "conexão: {0}", "conexión: {0}", "连接：{0}")
        , ("aiContextTarget", "visual target: {0}", "destino visual: {0}", "destino visual: {0}", "可视目标：{0}")
        , ("aiContextFields", "known fields in results: {0}", "campos conhecidos nos resultados: {0}", "campos conocidos en los resultados: {0}", "结果中的已知字段：{0}")
        , ("aiContextSafety", "do not execute queries or alter the Explorer during review", "não executar consultas nem alterar o Explorer durante a revisão", "no ejecutar consultas ni alterar el Explorer durante la revisión", "审查期间不要执行查询或修改 Explorer")
        ...
        , ("aiCatalogContextContract", "The model requires context contract \"{0}\", which this SlopStudio version does not support. Supported contracts: {1}.", "Modelo requer contrato de contexto \"{0}\", não suportado por esta versão do SlopStudio. Contratos suportados: {1}.", "El modelo requiere el contrato de contexto \"{0}\", no compatible con esta versión de SlopStudio. Contratos compatibles: {1}.", "模型需要上下文契约“{0}”，此版本的 SlopStudio 不支持。支持的契约：{1}。")
        ...
        , ("aiChatChanged", "I analyzed the editor and MongoDB context. The proposal adds only the requested filters and limit; review the code before applying.", "Analisei o editor e o contexto MongoDB. A proposta adiciona apenas os filtros e o limite solicitados; revise o código antes de aplicar.", "Analicé el editor y el contexto de MongoDB. La propuesta solo añade los filtros y el límite solicitados; revise el código antes de aplicarla.", "我分析了编辑器和 MongoDB 上下文。建议仅添加请求的筛选条件和限制；应用前请检查代码。")
        , ("aiChatUnchanged", "I analyzed the instruction and current content. I found no safe, unambiguous transformation; the proposal keeps the editor for your review.", "Analisei a instrução e o conteúdo atual. Não encontrei uma transformação segura e inequívoca; a proposta mantém o editor para sua revisão.", "Analicé la instrucción y el contenido actual. No encontré una transformación segura e inequívoca; la propuesta conserva el editor para su revisión.", "我分析了指令和当前内容。未找到安全且明确的转换；建议保留编辑器内容供您检查。")
        , ("aiChatRiskWarning", "The proposal contains a write or destructive operation. Applying it requires additional confirmation and will not execute the query.", "A proposta contém uma operação de escrita ou destrutiva. A aplicação exigirá uma confirmação adicional e não executará a consulta.", "La propuesta contiene una operación de escritura o destructiva. Aplicarla requiere confirmación adicional y no ejecutará la consulta.", "建议包含写入或破坏性操作。应用需要额外确认，且不会执行查询。")
        , ("aiChatBasicReason", "Basic mode; no ONNX inference. ", "Modo básico, sem inferência ONNX. ", "Modo básico, sin inferencia ONNX. ", "基础模式；不使用 ONNX 推理。")
        , ("aiChatDisabledReason", "Local AI is disabled for the Assistant; no ONNX inference. ", "IA local desabilitada para o Assistente, sem inferência ONNX. ", "La IA local está deshabilitada para el Asistente; no hay inferencia ONNX. ", "助手的本地 AI 已禁用；不使用 ONNX 推理。")
        , ("aiChatNoModelReason", "No local model selected; no ONNX inference. ", "Nenhum modelo local selecionado, sem inferência ONNX. ", "No hay ningún modelo local seleccionado; no hay inferencia ONNX. ", "未选择本地模型；不使用 ONNX 推理。")
        ...
        , ("aiChatContextTooLarge", "The context is too large for a local proposal. Reduce the content before requesting AI.", "Contexto grande demais para uma proposta local. Reduza o conteúdo antes de solicitar à IA.", "El contexto es demasiado grande para una propuesta local. Reduzca el contenido antes de solicitar a la IA.", "上下文对于本地建议过大。请减少内容后再请求 AI。")
        , ("aiChatSensitiveContext", "The context contains possible secrets or reserved markers. Remove them before requesting AI.", "O contexto contém possíveis segredos ou marcadores reservados. Remova-os antes de solicitar à IA.", "El contexto contiene posibles secretos o marcadores reservados. Elimínelos antes de solicitar a la IA.", "上下文包含可能的秘密或保留标记。请删除后再请求 AI。")
        , ("aiChatSettingsChanged", "AI preferences changed.", "As preferências de IA mudaram.", "Las preferencias de IA cambiaron.", "AI 首选项已更改。")
        , ("aiChatInvalidProposal", "The model did not produce a complete, valid proposal. Check the model, hardware and context limit.", "O modelo não produziu uma proposta completa e válida. Confira o modelo, o hardware e o limite de contexto.", "El modelo no produjo una propuesta completa y válida. Compruebe el modelo, el hardware y el límite de contexto.", "模型未生成完整有效的建议。请检查模型、硬件和上下文限制。")
        , ("aiChatGenerated", "Code proposal generated by local ONNX model {0} (FIM). Review the diff before applying.", "Proposta de código gerada pelo modelo ONNX local {0} (FIM). Revise o diff antes de aplicar.", "Propuesta de código generada por el modelo ONNX local {0} (FIM). Revise el diff antes de aplicarla.", "本地 ONNX 模型 {0}（FIM）生成的代码建议。应用前请检查差异。")
        , ("aiChatLocalRiskWarning", "The proposal contains a write or destructive operation; it requires additional confirmation and will not execute the query.", "A proposta contém escrita ou operação destrutiva; exige confirmação adicional e não executa a consulta.", "La propuesta contiene una escritura u operación destructiva; requiere confirmación adicional y no ejecuta la consulta.", "建议包含写入或破坏性操作；需要额外确认，且不会执行查询。")
