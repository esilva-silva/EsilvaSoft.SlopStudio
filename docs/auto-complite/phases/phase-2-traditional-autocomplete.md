...
| 2.4 | Integrado no núcleo automatizado | `CompletionContextCache` memoiza por identidade completa do snapshot, cursor, dialeto, escopo e gatilho, e a aba reutiliza sua versão enquanto o texto não muda. Uma lista visível é reconsultada em `Peek` quando `MetadataCache.Changed` afeta o perfil/banco/coleção da aba, preservando `SymbolId`; digitação não agenda carga. `PipelineInfo`/`ShapeWalker` cobrem estágios preservadores, projeção, grupo, lookup, facet, unwind, replaceRoot e count; permanecem cenários estrangeiros de schema e homologação nativa fora desta meta |
| 2.4 Completion/ranking | Funcional no núcleo | `CompletionService`, provider tradicional, ranking determinístico, snippets LSP, uso em memória, shapes semânticos, quotas entre fontes, tipos BSON, namespaces de metadata, `$lookup` estrangeiro e integração com schema aprendido estão implementados. Corpus focado de linguagem/ranking/edição: 675 fixtures + gate agregado aprovados. O gate publica MRR 1,000, top-1 44/44 e top-5 44/44; performance permanece fora desta meta |
| 2.4 | Funcional | Ranker usa top-K parcial determinístico, preserva destaques, filtros por shape/tipo, prioridade de leitura/escrita, `TypeMismatchPenalty` estruturado e uso integrado quando há identidade completa. O conjunto-ouro local e as métricas funcionais foram publicados sem executar benchmark |
| 2.5 | Funcional no fluxo automatizado; UI nativa fora do escopo | A lista contextual usa `CompletionWindowPresenter`, seleção por `SymbolId`, refiltro, ↑/↓/Enter/Tab/Esc, snippets, documentação tardia e cancelamento por geração. Os testes automatizados cobrem o fluxo; acessibilidade profunda, layouts nativos e matriz visual completa permanecem fora desta meta |
| 2.6 | Integrado | `CompletionContextEngine`, `ShapeWalker`, catálogo e `InlineCompletionCoordinator` atendem lista explícita e ghost determinístico/IA via políticas compartilhadas. O caminho legado permanece somente onde necessário para o pipeline de IA; não há remoção arriscada sem a validação nativa excluída desta meta |
