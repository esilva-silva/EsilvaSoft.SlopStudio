...
F5 executa tudo; Ctrl+Enter executa seleção ou tudo; Ctrl+Espaço abre a lista contextual tradicional (Ctrl+; é reconhecido para IA, mas ainda sem fluxo implementado; um override salvo em Ctrl+. continua funcionando, sem ser mais o padrão); F6 permite sair do editor; Ctrl+Tab alterna abas. Busca avançada e folding continuam pendentes; highlighting e resultados JSON/árvore já têm implementação, conforme o inventário atual. Rascunhos recuperam texto/contexto; políticas de recuperação e entrada JSON estão no [design system](17-design-system-ui-ux.md).
