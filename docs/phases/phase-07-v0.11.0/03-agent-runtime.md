...
**Contrato aditivo P7-CL3-05 (25/09/2026, implementado; homologação pendente).** Três membros opcionais em `IAgentSession`, com padrão que preserva o comportamento anterior:

- `ObservableNativeTools`: nomes de ferramentas nativas que o próprio provider executa fora do registry (Claude Code: `Read`/`Glob`/`Grep`). Lidos uma vez ao iniciar a sessão e saneados (token ASCII ≤ 64, fora do namespace `mcp*`, diferente — sem distinção de maiúsculas — de qualquer tool do registry). Só para esses nomes, `ToolStarted`/`ToolCompleted`/`ToolFailed` do adapter viram **observação de exibição**: o runtime publica `ToolRequested`+`ToolStarted` e um terminal com **ID gerado pelo runtime** e `AgentEvent.ToolOrigin = ProviderObserved` (tools do registry levam `Registry`). Evento com argumentos, texto que não seja código seguro, `MessageId`/`ApprovalId`, ID de chamada do registry, duplicado, órfão, renomeado ou acima de `MaxObservedToolsPerTurn` (64) é descartado sem falhar o turno. Observação nunca é respondível por `SubmitToolResultAsync`, não despacha, não pede nem concede aprovação e não altera o desfecho do turno; aberta no fim do turno, fecha como `ToolFailed`/`OutcomeUnknown` (`ObservedToolUnconfirmed`).
- `GetCancellationReport(turnId)`: consultado somente depois do dreno confirmado, quando o próprio runtime parou o turno (usuário, consumidor, sessão fechada, consumidor indisponível). **Precedência:** interrupção não confirmada, tool incerta, falha, fim natural e `TimedOut` são decididos antes; o relato só pode transformar `Cancelled` em `OutcomeUnknown` (`CancelledAfterSend`), nunca tornar um desfecho mais limpo. Relato que lança exceção ou valor indefinido conta como `MayHaveTakenEffect`. Claude Code relata `MayHaveTakenEffect` a partir do instante em que o prompt começa a ser escrito no stdin (turno retomável por `--resume`) e `NothingSent` antes disso.
- `ProviderErrorCodes`: códigos tipados e fixos que o adapter pode pôr em `AgentError`; só identificadores ASCII curtos são aceitos. Código declarado chega à UI como código do turno (ex.: `ClaudeCodeNotLoggedIn`); qualquer outro texto continua `ProviderError`. O turno falha do mesmo jeito.
...
Cancelamento tem dois caminhos: sinalizar CTS específica e enviar interrupção oficial ao provider. Se o adapter não confirmar parada no prazo proposto de 5 s, fechar seu processo/canal isolado e reportar interrupção. Operação já enviada ao MongoDB pode ter sido aplicada; retornar `OutcomeUnknown`, nunca rollback. Cancelamento antes do despacho é `Cancelled` sem efeito. Separar prazo de aprovação (120 s) de execução MongoDB (padrão 5 s, teto 30 s). `maxTimeMS` começa no despacho e não pode exceder prazo restante.

**Orçamento por fase (decisão P7-L10-WIRE, 25/09/2026).** O teto único de 155 s deixa de valer para escrita com aprovação: o runtime rearma o prazo a cada fase e só devolve o orçamento de execução depois da decisão humana. Com a ponte de aprovação composta (`AgentRuntimeWriteApprovalBridge`), uma escrita tem até `ToolTimeout` (35 s) para intenção durável e preparo — o preparo do registry é limitado a 30 s —, a janela de aprovação (até 120 s, a mesma no coordenador e no runtime para que a falta de resposta seja auditada como expiração, nunca como recusa) mais 2 × `StopTimeout` (5 s) de tolerância e, só após aprovação, `ToolTimeout` para execução. O pior caso é `AgentRuntimeOptions.MaxToolCallDuration` = 35 + 120 + 10 + 35 = **200 s**; sem a ponte a escrita fica em `ToolTimeout + ApprovalTimeout` (155 s) e a leitura em `ToolTimeout` (35 s). A espera de um adapter por resultados de tool precisa cobrir esse pior caso mais 30 s de folga, e a composição recusa valores menores, derivados das opções do runtime composto: Claude API 240 s, OpenAI 300 s. Os watchdogs do stream (`StreamIdleTimeout`, `RoundTimeout`) valem só durante a leitura de cada rodada, não durante a espera por resultados de tool. O turno tem orçamento próprio configurado e pode encerrar antes desses tetos. Não repetir escrita automaticamente por timeout/reconexão.
