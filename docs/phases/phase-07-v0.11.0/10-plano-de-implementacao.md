...
| 1 — Credenciais e principal autenticado | 0 | Parcial: Application `ISecretStore` e `IAgentCredentialProvider`; Core `SecretReference`; Infrastructure `WindowsCredentialSecretStore` e `LinuxSecretServiceSecretStore` isolados. Pendentes: `AgentCredentialProvider`, DI, identidade e migração versionada. | Windows: 30 testes determinísticos aprovados e 1 round-trip nativo ignorado por `1312 / NoLogonSession`. Linux: 84 testes focados aprovados, sem D-Bus/Secret Service nativo. Ainda exigir disponibilidade/ausência/bloqueio reais, falhas da migração, reinício, exclusão e concorrência nos dois SOs; sem fallback plaintext e com perfis legados recuperáveis. |
| 2 — Registry, autorização e auditoria | 1 | Parcial: contratos de política/permissão, avaliador e registry interno com `list_connections`. Pendentes: DI, schemas fechados, controle de saída, auditoria durável, executor BSON literal e todos os demais handlers. | Resultado focado combinado do incremento atual: 136 aprovados e 1 ignorado. Ainda exigir entradas extras/ENV/JS negadas, namespace e read-only, negação de saída, auditoria indisponível, mesmo handler/política para todos os ingressos, schemas versionados e fixtures por tool. Nenhuma tool é exposta por MCP. |
