...
**Planejado.** Nenhuma etapa abaixo foi implementada nesta meta documental. Prefixo de projetos omitido nas tabelas: `EsilvaSoft.SlopStudio`. Cada lote deve terminar com revisão de diff, documentação e evidência do seu gate antes de liberar o seguinte.
...
| 1 — Credenciais e principal autenticado | 0 | Application: `ISecretStore`, `IAgentCredentialProvider`; Infrastructure: `WindowsSecretStore`, `LinuxSecretStore`, `AgentCredentialProvider`, migração versionada; Core: `SecretReference`, `AgentPrincipal` | Cofre disponível/ausente/bloqueado, falha em cada etapa da migração, reinício, exclusão e concorrência. Zero credenciais plaintext novas; falha visível sem fallback; perfis legados recuperáveis |
| 2 — Registry, autorização e auditoria | 1 | Application: `IAgentToolRegistry`, `IAgentPermissionService`, `IAgentApprovalService`, `AgentToolRegistry`, `AgentPermissionService`; Infrastructure: executor BSON literal e repositório aditivo de auditoria | Inputs extras/ENV/JS negados, namespace e read-only, negação de saída, auditoria indisponível. Mesmo handler e política para todos os ingressos; schemas versionados e fixtures por tool |
...
| 7 — Adapter OpenAI/Codex | 0,1,5,6; 3 quando usa ponte MCP | Infrastructure.Agents/OpenAI: `OpenAiAgentProvider`, `CodexAgentSession`, `CodexAppServerClient`; composition root | Login oficial/cancelado/expirado, API Key, streaming, tools, interrupção e falha de processo. Provar confinamento e armazenamento; se impossível, API direta é fallback com alteração explícita de capabilities/aceite, nunca declarar login ChatGPT entregue sem evidência |
...
| --- | --- | --- |
| App Server permite confinamento sem tools alternativas? | Arquitetura/segurança, lote 0 | Relatório reproduzível; se não, provider API com auth/capabilities reduzidas e aceite revisado, sem prometer equivalência |
