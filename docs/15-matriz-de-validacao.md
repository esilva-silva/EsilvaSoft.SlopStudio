...
| Conta Claude — modo Claude Code (bloco CL; substitui 8B em 25/09/2026; escopo de ferramentas revisado no mesmo dia — [ADR-054 revisada](10-decisoes-arquiteturais.md#revisão-de-25092026-mesma-data-decisão-posterior-do-usuário--riscos-residuais-do-threat-model-não-aceitos-escopo-revertido)) | Spike P7-CL0-01 **executado no Windows** ([relatório](../eng/spikes/phase-07/claude-code/README.md)); CC-01..08 automatizados só com CLI falso/fixtures; GCL-1..8; casos manuais C-01..C-36 por SO/versão do Claude Code/plano: detecção, login/logout delegados, auth expirada, API Key no ambiente bloqueando, modo em uso, **ferramenta ausente por construção** para execução/escrita/rede (C-08..C-11, C-26, C-32, C-33), leitura com/sem pasta de workspace (C-35/C-36), cancelamento só por Job Object/grupo de processos, resume, zero tokens nos artefatos do Slop. [Plano](phases/phase-07-v0.11.0/23-integracao-claude.md) | Manual pelo usuário com conta própria; **não implementado, não executado; AC-03/06/07/08/09/10/11/12/14/15/17 pendentes**; ferramentas nativas de execução/escrita/rede **não liberadas** (riscos residuais não aceitos pelo usuário); leitura pura e tools do produto via MCP até o [threat model — documento 22](phases/phase-07-v0.11.0/22-threat-model.md) (GCL-3 pendente) |
