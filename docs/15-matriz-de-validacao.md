...
`ISecretStore` e `IAgentCredentialProvider` foram adicionados em Application; `SecretReference` opaca/versionada fica em Core. `WindowsCredentialSecretStore` isolado em Infrastructure: **30 testes determinísticos aprovados** e **1 round-trip nativo ignorado** por `1312 / NoLogonSession` (reconfirmado no Windows em 23/09/2026). `LinuxSecretServiceSecretStore` isolado: **84 testes focados aprovados**, sem D-Bus/Secret Service nativo; nenhuma execução Linux foi feita nesta continuação. A persistência de política/grants via proprietário LiteDB, evaluator e registry interno de `list_connections` estão implementados; provider e repository de política e evaluator estão registrados no DI, compartilhando o único owner. A suíte focada Windows de composição, evaluator, registry e persistência passou com **52 aprovados, 0 ignorados** em 23/09/2026. Estado: **implementação parcial; sem ingress MCP/runtime, auditoria durável, controle completo de saída, ferramenta exposta ou aprovação de qualquer AC**. Não comprova provider real, protocolo MCP ou homologação nativa completa.
