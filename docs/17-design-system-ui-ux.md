...
## IA local: consentimento, prévia e proposta — Fase 5, 22/09/2026 (painel removido em 25/09/2026)

> **Removido em 25/09/2026 (P7-L06-CLEANUP, [ADR-055](10-decisoes-arquiteturais.md#adr-055--remoção-do-assistente-ia-por-aba-25092026)).** A coluna de 240 do Assistente IA dentro da aba deixou de existir: `WorkspaceTabView` volta a ser só cabeçalho de destino + editor/resultados em toda a largura, e o chat da aba é o painel **Agente IA** (seção [Hospedagem na janela principal](#hospedagem-na-janela-principal-p7-l06-host--25092026)). Os opt-ins de contexto local descritos abaixo saíram das preferências e do cadastro de conexão; os valores persistidos são preservados sem consumidor (pendência de produto no [doc 16](phases/phase-07-v0.11.0/16-chat-nativo.md)). O texto a seguir é histórico e as capturas `ai-chat-localization` não são mais geradas.
