...
**Decisão:** ISecretStore assíncrono, Credential Manager por usuário no Windows e Secret Service no Linux; referências no banco. Cofre indisponível gera falha ou memória explícita, nunca fallback plaintext. Login Codex via App Server fica pendente de suporte de produção, keyring real e confinamento comprovados; Slop não copia refresh tokens. API OpenAI direta é a baseline inicial e usa API Key no cofre, sem alegar login de assinatura. Claude embutido usa API Key; login de assinatura só após autorização oficial específica. Falhas do cofre incluem `Cancelled` quando o usuário dispensa um prompt de desbloqueio. `Tmds.DBus.Protocol` 0.94.1 é candidato Linux de baixo nível, ainda sem execução D-Bus nesta validação. Migração aditiva grava/verifica cofre antes de trocar referência, com recuperação após queda. [Fluxos e fontes](phases/phase-07-v0.11.0/07-autenticacao-e-segredos.md).
