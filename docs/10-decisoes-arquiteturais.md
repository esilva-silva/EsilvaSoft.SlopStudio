...
**Revisado por ADR-053/054 (25/09/2026):** o sublote 8B citado acima é substituído pelo bloco prioritário Integração Claude ([plano 10](phases/phase-07-v0.11.0/10-plano-de-implementacao.md#bloco-prioritário--integração-claude-25092026), [23](phases/phase-07-v0.11.0/23-integracao-claude.md)); a modalidade fixada é o subprocesso do binário oficial `claude` (Agent SDK em sidecar rejeitado). A proibição geral de "shell/arquivos/subagents" desta ADR permanece para todos os providers, exceto no modo Claude Code, onde o ADR-054 libera as ferramentas nativas do binário oficial mediante aprovação por chamada — gate GCL-3/GCL-5, bloqueado até o [threat model (22)](phases/phase-07-v0.11.0/22-threat-model.md).
...
**Revisado por ADR-053/054 (25/09/2026):** o ADR-054 revisa esta ADR-051 **somente no modo Claude Code**. A frase "Shell/file tools dos providers permanecem desabilitadas para evitar contorno" continua valendo para todos os outros providers (API Anthropic, OpenAI, Codex). No modo Claude Code, ferramentas nativas do binário oficial passam a ser liberadas mediante aprovação por chamada (ADR-054), mas os controles read-only, os grants do registry e a auditoria do Slop descritos nesta ADR **não confinam** essas ferramentas nativas: o único controle sobre elas é o diálogo de aprovação por chamada mais as regras de negação determinísticas que o [threat model (22)](phases/phase-07-v0.11.0/22-threat-model.md) ainda precisa definir e comprovar (gate GCL-3/GCL-5). Um comando aprovado roda como o usuário do sistema operacional e pode, por exemplo, ler o cofre do SO ou arquivos fora do escopo do registry — risco residual explícito, não uma extensão da fronteira desta ADR.

## ADR-052 — Consentimento local e prévia do contexto de chat (22/09/2026)

**Estado: implementada no escopo automatizado da Fase 5.** Contexto: a inferência roda no dispositivo, mas o código, destino e Input JSON continuam sendo dados escolhidos pela pessoa; consentir em usar o Assistente não deve implicar transmissão invisível.
